...
<body>
    ...
    <div class="main-title">
        <h2 id="seat-id" data="<?php echo ($list["id"]); ?>"><?php echo ($modular); ?></h2><?php echo session("flash_error");?>
        <?php echo ($er); ?>
    </div>
    ...
    <form onsubmit="return checkInput();" action="<?php echo U();?>" method="post" class="form-horizontal" enctype="multipart/form-data">
        ...
        <div class="form-item effect_img_div" style="width:100%;height:100px;">
            <div style="width:200px;height:100px;float:left;overflow:hidden;">
                <form>
            ...
            <div style="display: inline-block;" id="into_effect_img">
                <?php if($effect_img != '' ): if(is_array($effect_img)): foreach($effect_img as $key=>$vo): ?><div class="effect_img_div" style="display: inline-block;">
                            <div class="button">x</div>
                            <img theme="<?php echo ($vo["id"]); ?>" class="old_effect_img" src="/microweb/Uploads/<?php echo ($vo["savepath"]); ?>/<?php echo ($vo["savename"]); ?>">
...
<script type="text/javascript">
    ...
    $(function() {
        ...
        $('#file_upload').uploadifive({
            'multi'    : false,
            'auto'             : true,
            'checkScript'      : '/microweb/Public/Static/uploadifive/check-exists.php',
            'formData'         : {
            ...
            'queueSizeLimit'   : img_num,
            'uploadScript'     : '/microweb/index.php/Admin/Reception/upload/id/'+$id+'/old_img/'+res,
            'onUploadComplete' : function(file, data) {
                $(".uploadifive-queue-item").remove();
                var res_data = JSON.parse(data);
                if(res_data.theme_id=="no"){
                    alert("已存在");
                    window.updateAlert("已存在" ,'alert-error');
                }else if(res_data=='false'){
                    aletr("上传失败");
                    window.updateAlert("上传失败" ,'alert-error');
//                    var that = document.getElementById("into_effect_img");
//                    $(".uploadifive-queue-item").remove();
//                    var res_data = JSON.parse(data);
//                    var urlData = '/microweb/Uploads/'+res_data.Filedata.savepath + res_data.Filedata.savename;
//                    that.innerHTML += "<div class='effect_img_div' style='display: inline-block;'>" +
//                            "<div class='button' style='display: none;'>x</div>" +
//                            "<img theme="+res_data.theme_id+" class='effect_img' src='" + urlData + "'/></div>";
//                    if(res){
//                        $("#into_effect_img").find(".effect_img_div").eq(0).remove();
//                    }else{
//                        num++;
//                    }
//                    if(num==3){
...
//                    }
    ...
    $(document).on("click",".button",function(){
        var that = $(this);
        var theme_id = that.next().attr("theme");
        $.post("/microweb/index.php/Admin/Reception /delete_theme",{theme_id:theme_id},function(data){
            if(data=="ok"){
                window.location.reload();
            }
        })
