...
class SiteInfoModel extends Model{
	protected $_validate = array(
		array('site_name', '/^([\x{4e00}-\x{9fa5}A-Za-z0-9_]){2,10}+$/u', '网站名为2到20位汉字、字母、数字的组合'),
		...
		array('site_name', 'checkName', '以存在此网站名', 0, 'callback', 3),
	...
	protected function checkName(){
		$name = I('post.site_name');
		$user_info = session('user_info');
		$result = $this->where(array('status' => 0,'site_name' => $name, 'user_id' => $user_info['id']))->find();
		return empty($result);
	}
